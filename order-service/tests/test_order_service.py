...
class TestCheckoutHappyPath:
    ...
    async def test_checkout_creates_confirmed_order(self, db):
        ...
        assert result.order.status == OrderStatus.pending_payment
        assert result.order.user_id == user_id
    ...
    async def test_checkout_locks_price_from_product_service(self, db):
        ...
        assert result.order.total_amount == Decimal("16.99")
        assert result.order.items[0].unit_price == Decimal("16.99")
    ...
    async def test_checkout_records_price_change(self, db):
        ...
        assert result.price_changes[0]["old_price"] == "14.99"
        assert result.price_changes[0]["new_price"] == "16.99"
    ...
    async def test_checkout_calculates_correct_total(self, db):
        ...
        assert result.order.total_amount == Decimal("44.97")
    ...
    async def test_checkout_creates_order_items(self, db):
        ...
        assert result.order.items[0].subtotal == Decimal("29.98")
...
class TestGetAndListOrders:
    ...
    async def test_get_order_returns_own_order(self, db):
        user_id = _uuid()
        checkout_result = await self._place_order(db, user_id)
        result = await OrderService.get_order(db, checkout_result.order.id, user_id)
        assert result is not None
        assert result.id == checkout_result.order.id

    async def test_get_order_returns_none_for_other_users_order(self, db):
        user_a = _uuid()
        user_b = _uuid()
        checkout_result = await self._place_order(db, user_a)

        # user_b tries to access user_a's order
        result = await OrderService.get_order(db, checkout_result.order.id, user_b)
