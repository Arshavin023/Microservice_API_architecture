...
async def deliver_shipment(
    ...
    """
    Internal/cron use — directly marks a dispatched shipment as delivered,
    bypassing the customer-confirmation flow. Used by the auto-confirm job
    that runs after 2 hours if the customer hasn't responded.
    Publishes shipment.delivered → order moves to 'delivered'.
    """
    uid = _parse_uuid(shipment_id, "shipment_id")
    try:
